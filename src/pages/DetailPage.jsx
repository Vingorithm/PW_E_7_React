...
const Detail = () => {
  ...
  const fetchCarDetail = useCallback(async () => {
    try {
      ...
      setBid(
        Math.max(
          parseFloat(response.data.data.starting_price) + 1000,
          response.data.data.starting_price
        )
      );
    } catch (error) {
  ...
  useEffect(() => {
    const calculateTimeLeft = () => {
      ...
      const [hours, minutes, seconds] = productDetail.end_time
        .split(":")
        .map(Number);
  ...
  const handleBidNowCardClick = useCallback(
    (carId) => {
      setLoading(true);
  ...
  const placeBid = async () => {
    ...
    try {
      ...
      const response = await CreateBid(inputData);
      ...
      if (error.response) {
        switch (error.response.status) {
          ...
          case 422:
            const errorMessage =
              error.response.data?.message || "Invalid bid data";
    ...
    <div className="container mt-5">
      <div className="row align-items-stretch">
        ...
        <div className="col-md-4 d-flex flex-column">
          <h1 className="mb-3">
            <strong>
              {productDetail.car?.brand} {productDetail.car?.model}
            </strong>
          </h1>
          ...
          <p style={{ fontSize: "32px" }}>
            {formatCurrency(productDetail.starting_price)}
          </p>
          <p>
          ...
          <div className="d-flex align-items-center">
            <button
              ...
              disabled={
                loading ||
                bid <= parseFloat(productDetail.starting_price) + 1000
              }
            >
              -
            </button>
                ...
                <div className="card h-100 shadow-sm">
                  ...
                  <div className="card-body d-flex flex-column">
                    ...
                    <button
                      ...
                      disabled={
                        product.auction?.status === "Ongoing" || loading
                      }
                      onClick={() =>
                        handleBidNowCardClick(product.auction?.car?.id)
                      }
                    >
